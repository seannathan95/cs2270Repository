...
bow_transform = BoWTransform(window_size=15, word_size=10, num_letters=4,
                strat='normal')
bow_transform.fit(X_train, y_train)
tfidf = bow_transform.tfidf_
vocabulary_length = len(bow_transform.vocabulary_)
X_new = bow_transform.decision_function(X_test)
...
plt.xticks(np.arange(vocabulary_length),
           np.vectorize(bow_transform.vocabulary_.get)(np.arange(vocabulary_length)),
           fontsize=14)
plt.ylim((0, 7))
